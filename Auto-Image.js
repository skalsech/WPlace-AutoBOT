...
; (async () => {
  ...
  const TEXT = {
    en: {
      ...
      colorsUpdated:
        '✅ Success available colors increased {oldCount} -> {newCount}, {diffCount} new colors found',
      ...
      noChargesThreshold: '⌛ Waiting to reach {threshold} charges. Estimated time: {time}.',
    },
    ru: {
    ...
    maybeNotifyChargesReached(force = false) {
      ...
      if (reached) {
        ...
        if (shouldEdge || shouldRepeat) {
          const msg = `Charges ready: ${Math.floor(
            state.currentCharges
          )} / ${state.maxCharges}. Threshold: ${state.cooldownChargeThreshold}.`;
  ...
  function initializeColorPalette(container) {
    ...
    if (!state.availableColors || state.availableColors.length === 0) {
      ...
      colorsContainer.innerHTML =
        '<div class="wplace-colors-placeholder">Upload an image first to capture available colors</div>';
      return;
    }
    ...
    if (settingsBtn && closeSettingsBtn && applySettingsBtn) {
      ...
      if (batchModeSelect) {
        batchModeSelect.addEventListener('change', (e) => {
          ...
          Utils.showAlert(
            `Batch mode set to: ${
              state.batchMode === 'random' ? 'Random Range' : 'Normal Fixed Size'
            }`,
            'success'
          );
        });
      ...
      if (state.colorsChecked) {
        colorSwatchesHTML = state.availableColors
          .map((color) => {
            ...
            return `<div class="wplace-stat-color-swatch" style="background-color: ${rgbString};" title="${color.name}\nID: ${color.id}\nRGB: ${color.rgb.join(
              ', '
            )}"></div>`;
          })
          .join('');
          ...
          for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
              ...
              const r = data[i4],
                ...
                a = data[i4 + 3];
              const isEligible =
                a >= tThresh && (state.paintWhitePixels || !Utils.isWhitePixel(r, g, b));
